// palindroma
const parolaUtente = "paolo"
palidromaValida = false;
let numeroPU;
palidroma();
console.log(palidromaValida);
console.log(parolaUtente[0], numeroPU);
function palidroma() {
  for (let i = parolaUtente.length - 1; i >= 0; i--) {
    console.log(parolaUtente[i]);
    numeroPU = parolaUtente.length - 1;
    if (parolaUtente[2] != numeroPU.toString) {
      palidromaValida = true
    }else{
      palidromaValida = false
    }
  }
